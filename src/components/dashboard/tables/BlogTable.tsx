...
import type { BlogData } from "@/components/dashboard/forms/BlogForm";
...
export default function BlogTable({
  ...
  const [totalPages, setTotalPages] = useState(() => {
    ...
    return Math.max(1, Math.ceil(initialBlogs.length / limit));
  });
  ...
  useEffect(() => {
    ...
    const shouldFetch = hasSearch || initialBlogs.length === 0 || refreshKey !== 0 || page !== 1;
    const shouldShowLoader = shouldFetch || initialBlogs.length === 0;

    if (shouldFetch) {
      fetchBlogs(shouldShowLoader, controller.signal, searchTerm, page);
    } else {
      ...
      setTotalPages(Math.max(1, Math.ceil((initialTotal ?? initialBlogs.length) / limit)));
    }
    return () => controller.abort();
  }, [fetchBlogs, refreshKey, initialBlogs, searchTerm, onCountChange, initialTotal, page, limit]);
  ...
  useEffect(() => {
    ...
    setTotalPages(Math.max(1, Math.ceil((initialTotal ?? initialBlogs.length) / limit)));
  }, [initialBlogs, onCountChange, initialTotal, limit]);
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        setConfirmId(null);
        fetchBlogs(true, undefined, searchTerm, page);
      } else {
        toast.custom((t) => (
    ...
    <div className="bg-white rounded-2xl shadow-xl p-8 w-full">
     ...
     <div>
          ...
          <h1 className="text-3xl font-bold text-gray-800 mt-2">
            Blog <span className="text-[#BF1E2E] font-greatvibes text-[34px]">List</span>
          </h1>
        </div>
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 mt-6">
        ...
        <div className="flex items-center gap-2">
          ...
          <button
            ...
            className="px-3 py-2 rounded-md bg-[#BF1E2E] text-white text-sm font-semibold hover:bg-[#A81826] disabled:opacity-50 disabled:cursor-not-allowed"
          >
            Next
          </button>
        </div>
