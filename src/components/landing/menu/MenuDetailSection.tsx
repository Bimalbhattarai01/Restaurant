...
export default function MenuDetailSection({
  ...
  description,
  price,
  images = ["/assets/landing/menu/Dish.png"],
}: MenuDetailSectionProps) {
  const gallery = useMemo(() => (images.length > 0 ? images : ["/assets/landing/menu/Dish.png"]), [images]);
  const galleryMeta = useMemo(
    () => gallery.map((image) => ({ image, ...resolveImageSource(image) })),
    [gallery]
  );
  const formattedPrice = useMemo(() => `£${price.replace(/[^0-9.,-]/g, "").trim()}`, [price]);
        ...
        <div className="flex flex-col gap-4">
          <div className="flex items-start justify-between gap-6">
            ...
            <div className="relative">
              <div className="flex flex-col items-center justify-center rounded-full bg-[#BF1E2E] text-white h-[120px] w-[120px] shadow-lg p-2">
                <span className="text-[38px] font-bold font-inter leading-none">{formattedPrice}</span>
              </div>
